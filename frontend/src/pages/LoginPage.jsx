...
export default function LoginPage() {
  ...
  const [showSSOHelp, setShowSSOHelp] = useState(false);
  ...
  useEffect(() => {
    ...
    if (error) {
      if (error === 'google_unconfigured') {
        ...
        const providerName = error.startsWith('google') ? 'Google' : 'GitHub';
        const reasonStr = params.get('reason') || params.get('details') || '';
        toast.error((t) => (
          <div className="text-xs text-left">
            <span className="font-extrabold text-red-400 block mb-1">{providerName} Authentication Failed</span>
            {reasonStr ? `Reason: ${reasonStr}` : `The ${providerName} sign-in process was cancelled or failed.`}
            {error === 'google_failed' && (
              <span className="block mt-1.5 text-slate-400 font-semibold text-[10px] leading-relaxed">
                Tip: Expand the <strong>Google Login Troubleshooting</strong> panel below to resolve OAuth redirect errors.
              </span>
            )}
          </div>
        ), { duration: 10000 });
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 font-sans bg-mesh-cyber relative">
      ...
      <div className="w-full max-w-md animate-fadeIn z-10">
        ...
        {/* Center Card */}
        <div className="glass-card rounded-3xl border-slate-900/60 p-8 shadow-2xl relative overflow-hidden glow-indigo">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="text-xl font-black text-slate-100 mt-4 tracking-tight">Welcome Back</h1>
            <p className="text-[11px] text-slate-400 mt-1 font-semibold">Sign in to your simulated equity dashboard</p>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div>
              <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2">Email Address</label>
              <input
                {...register('email')}
                type="email"
                placeholder="name@company.com"
                className="w-full bg-slate-950/80 border border-slate-800 focus:border-cyan-500 rounded-xl px-4 py-2.5 text-slate-200 text-xs placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-cyan-500/20 transition-all font-sans"
              />
              {errors.email && <p className="text-red-400 text-[10px] font-semibold mt-1.5">{errors.email.message}</p>}
            </div>

            <div>
              <label className="block text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-2">Password</label>
              <div className="relative">
                <input
                  {...register('password')}
                  type={showPw ? 'text' : 'password'}
                  placeholder="••••••••"
                  className="w-full bg-slate-950/80 border border-slate-800 focus:border-cyan-500 rounded-xl px-4 py-2.5 text-slate-200 text-xs placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-cyan-500/20 transition-all font-sans pr-12"
                />
                <button
                  type="button"
              ...
              {errors.password && <p className="text-red-400 text-[10px] font-semibold mt-1.5">{errors.password.message}</p>}
            </div>

            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-cyan-500 to-emerald-500 hover:from-cyan-400 hover:to-emerald-400 text-slate-950 font-black text-xs py-3 rounded-xl transition duration-300 shadow-lg shadow-cyan-500/10 flex items-center justify-center mt-6"
            >
              {loading ? (
            ...
            <div className="mt-5 border-t border-slate-900/60 pt-4 flex flex-col gap-3">
              
              {/* Google Login Troubleshooting */}
              <div>
                <button
                  type="button"
                  onClick={() => setShowSSOHelp(!showSSOHelp)}
                  className="w-full flex items-center justify-between text-[10px] font-black text-emerald-400 hover:text-emerald-300 uppercase tracking-widest transition-colors focus:outline-none"
                >
                  <span className="flex items-center gap-1.5">
                    <span className="animate-pulse">⚡</span>
                    Google Login Troubleshooting
                  </span>
                  {showSSOHelp ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                </button>

                {showSSOHelp && (
                  <div className="mt-3.5 space-y-3 animate-fadeIn text-left bg-slate-950 p-4 border border-emerald-950/40 rounded-2xl glow-emerald">
                    <p className="text-[9px] text-slate-400 leading-normal font-semibold">
                      <span className="font-bold text-slate-200 block mb-1">Fixing Error 400: redirect_uri_mismatch:</span>
                      Google OAuth requires that the server callback URL is explicitly whitelisted in your Cloud Console. If you receive a redirect URI mismatch:
                    </p>
                    <ol className="list-decimal pl-4 mt-2 space-y-2 text-[9.5px] text-slate-400 font-semibold leading-relaxed">
                      <li>Go to the <a href="https://console.cloud.google.com/apis/credentials" target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:text-cyan-300 underline font-black">Google Cloud Console Credentials</a>.</li>
                      <li>Select your project and click Edit on your <strong>OAuth 2.0 Client ID</strong>.</li>
                      <li>Scroll down to the <strong>Authorized redirect URIs</strong> section.</li>
                      <li>Add the exact backend callback URL:<br />
                        <code className="text-emerald-400 bg-slate-900 border border-slate-800 px-1.5 py-0.5 rounded select-all font-mono inline-block mt-1">http://localhost:5000/api/auth/google/callback</code>
                      </li>
                      <li>Click <strong>Save</strong> and wait 2-3 minutes for Google servers to update before retrying!</li>
                    </ol>
                  </div>
                )}
              </div>
          ...
          <p className="text-center text-slate-400 text-xs font-semibold mt-6">
            Don't have a login?{' '}
            <Link to="/register" className="text-emerald-400 hover:text-emerald-300 font-bold underline underline-offset-2 transition-colors">
              Create Free Account
            </Link>
          </p>
        </div>
